...
from data.extract import (
    ...
    EXTRACT_PATH, USAGE_PATH, BRIDGE_PATH, STUDY_PATH, USER_GROUP_PATH, QMNEM_PATH
)
from components.worksheet import (
    build_layout, build_table_panel, build_middle_panel,
    AVAILABLE_FIELDS, build_query, get_field_expr,
...
HIGH_CARDINALITY_FIELDS = {"LONG_NAME", "EXT_STUDY_ID", "STUDYID", "USER_NAME", "USER_EMAIL", "QMNEM"}

_STUDY_FILTER_FIELDS      = {"EXT_STUDY_ID", "LONG_NAME", "STUDYID", "STUDYYEAR"}
_USER_GROUP_FILTER_FIELDS = {"GROUP_NAME"}
_QMNEM_FILTER_FIELDS      = {"QMNEM"}
...
app.layout = html.Div([
    ...
    html.Button(id="show-sql-trigger-btn",      style={"display": "none"}),
   ...
   dbc.Modal([
    ...
    dbc.Modal([
        dbc.ModalHeader("📋 SQL Query"),
        dbc.ModalBody([
            html.Pre(id="sql-display-content",
        ...
        dbc.ModalFooter([
            dbc.Button("Copy", id="sql-copy-btn",
                      color="primary", size="sm", className="me-2"),
            dbc.Button("Close", id="sql-display-close-btn",
                      color="secondary", size="sm"),
        ]),
    ], id="sql-display-modal", is_open=False, size="lg"),
...
@app.callback(
    Output("sql-display-modal",     "is_open"),
    Output("sql-display-content",   "children"),
...
def display_sql(payload, close_clicks, ws_last_sql):
    ...
    if triggered == "sql-display-close-btn":
        return False, ""
    if triggered == "sql-display-payload" and payload:
        ws_key = payload.replace(" ", "_")
        sql = (ws_last_sql or {}).get(ws_key, "No SQL found for this worksheet.")
        return True, sql
    raise dash.exceptions.PreventUpdate
...
def run_worksheet_query(n_clicks, rows_data, cols_data, field_filters_data,
                        ...
                        ws_settings, last_run, ws_filters_data, ws_last_sql):
    ...
    if "QMNEM" in all_fields:
        # Check fields from rows/cols and field_filters
        filter_fields = {k.split("|")[-1] for k in field_filters.keys()}
        all_study_fields = set(all_fields) | filter_fields
        has_study_field = any(f in STUDY_FIELDS for f in all_study_fields)
        if not has_study_field:
            result = dbc.Alert(
                "⚠️ QMNEM requires at least one study field on Rows, Columns, or Filters.",
    ...
    if extract_available():
        for f in duck_fields:
            # Don't apply date formatting to fields fetched only for formulas
    ...
    try:
        if use_duckdb:
            ...
            if "Count" in df.columns and cols:
                ...
                if has_fanout:
                    summary_row_fields = [f for f in rows if f not in FANOUT_FIELDS and f not in formula_calcs]
...
def export_data(n_clicks, rows_data, cols_data, field_filters_data,
    ...
    try:
        ...
        if cols and rows and "Count" in df.columns:
            ...
            # Load pre-computed summary for correct Grand Total
            df_summary = None
            summary_path = results_dir / f"{ws_key}.summary.parquet"
            if summary_path.exists():
                try:
...
def export_crosstab(n_clicks, rows_data, cols_data, field_filters_data,
    ...
    try:
        ...
        if cols and rows and "Count" in df.columns:
            ...
            # ── Summary query for deduped column totals ───────────
            # Load pre-computed summary for correct Grand Total
            df_summary = None
            summary_path = results_dir / f"{ws_key}.summary.parquet"
            if summary_path.exists():
