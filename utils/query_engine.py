...
from data.extract import USAGE_PATH, BRIDGE_PATH, STUDY_PATH, USER_GROUP_PATH, QMNEM_PATH, EXTRACT_PATH
...
FIELD_REGISTRY = {
    ...
    # ── QMNEM field (qmnem.parquet via USAGE_ID) ────────────────────────────
    "QMNEM":        {"col": "q.QMNEM",        "table": "qmnem"},
...
QMNEM_FIELDS      = {k for k, v in FIELD_REGISTRY.items() if v["table"] == "qmnem"}
...
def _needs_qmnem_join(fields, where_clauses, measure_sql, global_calcs):
    """True if QMNEM field appears anywhere in the query."""
    for f in fields:
        if f in QMNEM_FIELDS:
            return True
    where_text = " ".join(where_clauses)
    for qf in QMNEM_FIELDS:
        if qf in where_text:
            return True
    if any(qf in measure_sql for qf in QMNEM_FIELDS):
        return True
    for defn in (global_calcs or {}).values():
        if isinstance(defn, dict):
            if defn.get("dim",   "") in QMNEM_FIELDS:
                return True
            if defn.get("field", "") in QMNEM_FIELDS:
                return True
    return False


def _build_from_clause(need_user_group, need_study, need_qmnem):
    """
    ...
    + QMNEM field:
        FROM read_parquet('usage.parquet') u
        [earlier joins as needed]
        JOIN read_parquet('qmnem.parquet')  q ON u.USAGE_ID = q.USAGE_ID
    ...
    if need_qmnem:
        clause += (
            f"\n        JOIN read_parquet('{qmnem_path}') q"
            f" ON u.USAGE_ID = q.USAGE_ID"
        )
...
def build_duckdb_query(fields, where_clauses, limit=None,
    ...
    need_qmnem       = _needs_qmnem_join(regular_fields, where_clauses, measure_sql, lod_calcs)
    from_clause      = _build_from_clause(need_user_group, need_study, need_qmnem)
